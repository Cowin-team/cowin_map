...
class City {
  ...
  fetchedAllData() {
    if (
      (this.availableResources.includes("beds") &&
        !this.hasFetchedCovidBedsData) ||
      (this.availableResources.includes("oxygen") &&
        !this.hasFetchedOxygenData) ||
      (this.availableResources.includes("meals") &&
        !this.hasFetchedMealsData) ||
    ...
    fetch(dataUrl)
      ...
      .then((data) => {
        data.forEach((dataRow) => {
          ...
          this.oxygenSupplyMarkers.push(
            new OxygenSupplyMarker(oxygenSupplyLocation)
          );
        });
      ...
      .catch((error) => {
        console.error(
          `!!!! Failed to fetch OXYGEN SUPPLY for ${this.name}: ${error}`
        );
        this.hasFetchedOxygenData = true;
      });
  }
